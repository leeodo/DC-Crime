...
va_violent_crime <- read.csv("data/va_dmv.csv") %>%
  slice(-1) %>%
...
graph <- ggplot(data = va_violent_crime, aes(
  x = Measures, fill = Measures,
  weight = Crime_rate_per_1000
)) +
  geom_bar() +
  scale_fill_brewer(palette = "RdPu", direction = -1) +
  labs(
    x = "Crime Type", y = "Crime Rate per 1,000",
...
save <- ggplotly(graph)
htmlwidgets::saveWidget(save, "html_viz/VA_2017_violent_crime.html")