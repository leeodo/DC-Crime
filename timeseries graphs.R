...
fig <- states_df %>% plot_ly(type = 'scatter', mode = 'lines', fill = 'tozeroy', fill = c('Green','Red','Yellow','Blue'), 
                             line = list(width = 4))
...
fig
fig <- fig %>% add_trace(x=~year[states_df$state=="District of Columbia"], y=~data_rates_violent_all[states_df$state=="District of Columbia"], name="District of Columbia",
                         line=list(color="#E81B39"), fillcolor = "E81B39")                        
fig <- fig %>% add_trace(x=~year[states_df$state=="Maryland"], y=~data_rates_violent_all[states_df$state=="Maryland"], name="Maryland",
                         line=list(color="#d5ab09"), fillcolor = "#d5ab09")   
fig <- fig %>% add_trace(x=~year[states_df$state=="Virginia"], y=~data_rates_violent_all[states_df$state=="Virginia"], name="Virginia",
...
myplot <- states_df %>%
  ...
  scale_color_manual(breaks = c("Maryland", "Virginia", "District of Columbia", "United States"),
  ...
  theme(
    plot.title = element_text(size = 27L,
                              face = "bold",
                              hjust = 0.5),
