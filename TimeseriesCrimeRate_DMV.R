...
fig <- states_df %>% plot_ly(type = 'scatter', mode = 'lines', fill = 'tozeroy', fill = c('#E81B39','#d5ab09','#00297B'), 
                             line = list(width = 4))
...
fig
fig <- fig %>% add_trace(x=~year[states_df$state=="District of Columbia"], y=~data_rate_all_crime[states_df$state=="District of Columbia"], name="District of Columbia",
                         line=list(color="#E81B39"), fillcolor = "E81B39")                        
fig <- fig %>% add_trace(x=~year[states_df$state=="Maryland"], y=~data_rate_all_crime[states_df$state=="Maryland"], name="Maryland",
                         line=list(color="#d5ab09"), fillcolor = "#d5ab09")   
fig <- fig %>% add_trace(x=~year[states_df$state=="Virginia"], y=~data_rate_all_crime[states_df$state=="Virginia"], name="Virginia",
